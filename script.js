...
,
 ...
 // Create separate html document for the following exercises:
// Use the array given below and perform the following operations using array methods:
// Note: Display the result using document.write()
// find()
...
superMarket.find((item)=>{
    if(item.height === 450 && item.width === 450){
        console.log(item)
    }
})
...
// 1
 console.log(superMarket.filter(item => item.rating>2))

 // 2

 console.log(superMarket.filter(item => item.type === 'dairy'))

 // 3

 console.log(superMarket.filter(item => item.price > 20))

// 4

superMarket.filter(item => item.title.split(' ').forEach((val)=>{
    if(val === 'with')
    console.log(item)
}))
...
superMarket.filter(item => item.description.split(' ').forEach((val)=>{
    if(val === 'with')
    console.log(item)
}))
...
console.log(superMarket.filter(item => item.height === 450 && item.width ==450))
...
let updated = superMarket.map((item)=>{
    return{
        ...item,
        isAvalable:"True"
    }
...
console.log((superMarket.reduce((preVal,curVal,curIndex,superMarket)=>{
...
// Mix of aray and methods


// 1

console.log(superMarket.filter((item)=>item.type==='dairy').reduce((preVal,curVal,superMarket)=>{
return preVal+curVal.rating
},0)/superMarket.filter((item)=>item.type==='dairy').length)

// 2

console.log(superMarket.filter((item)=>item.type==='vegetable').reduce((preVal,curVal,superMarket)=>{
return preVal+curVal.price
},0)/superMarket.filter((item)=>item.type==='vegetable').length)
